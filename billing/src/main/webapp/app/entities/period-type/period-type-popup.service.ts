            ...
            if (id) {
                this.periodTypeService.find(id)
                    .subscribe((periodTypeResponse: HttpResponse<PeriodType>) => {
                        const periodType: PeriodType = periodTypeResponse.body;
                        this.ngbModalRef = this.periodTypeModalRef(component, periodType);
                        resolve(this.ngbModalRef);
                    });
            } else {
                // setTimeout used as a workaround for getting ExpressionChangedAfterItHasBeenCheckedError
                setTimeout(() => {
                    this.ngbModalRef = this.periodTypeModalRef(component, new PeriodType());
                    resolve(this.ngbModalRef);
                }, 0);
            }
        });
